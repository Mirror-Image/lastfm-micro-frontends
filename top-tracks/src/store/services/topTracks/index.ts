import { createApi } from "@reduxjs/toolkit/query/react";

import { axiosBaseQuery } from "../axiosBaseQuery";

import { LIST_ID, TOP_ARTISTS_TAG } from "./constants.ts";
import { TGetTopTracksResponse, TGetTopTracksRequest } from "./types.ts";

import { ENV_LASTFM_API_KEY } from "@/constants/envs.ts";

export const topTracksApi = createApi({
  reducerPath: "topTracksApi",
  tagTypes: [TOP_ARTISTS_TAG],
  baseQuery: axiosBaseQuery({ baseUrl: "https://ws.audioscrobbler.com/2.0/" }),
  endpoints: (builder) => ({
    getTopTracks: builder.query<TGetTopTracksResponse, TGetTopTracksRequest>({
      query: ({ page, limit = 10 }) => ({
        url: `?method=chart.gettoptracks&api_key=${ENV_LASTFM_API_KEY}&format=json`,
        params: {
          page,
          limit,
        },
      }),
      providesTags: (result) =>
        result
          ? [
              ...(result?.tracks?.track.map(({ mbid }) => ({
                type: TOP_ARTISTS_TAG,
                id: mbid,
              })) ?? []),
              { type: TOP_ARTISTS_TAG, id: LIST_ID },
            ]
          : [{ type: TOP_ARTISTS_TAG, id: LIST_ID }],
    }),
  }),
});

export const { useGetTopTracksQuery } = topTracksApi;
